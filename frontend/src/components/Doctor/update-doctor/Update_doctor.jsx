...
import "./Update_doctor.css";
import DoctorImage from "../../../img/Update-pana.svg";
import Navbar from "../../navbar/navbar";
import SuccessLogo from "../../../img/success.png";

import Message from "../../Message/Message";

const Update_doctor = ({ state }) => {
  console.log(state);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const res = await fetch("http://localhost:4000/update-doctor", {
        method: "POST",
        headers: {
      ...
      if (data.status === 200) {
        ...
        setMessage({
          type: "error",
          text: "Doctor License ID is not eligible for update. Please use a different License ID.",
        });
        clearForm();
      } else {
