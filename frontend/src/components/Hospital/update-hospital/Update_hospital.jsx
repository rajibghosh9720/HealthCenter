...
const Update_Hospital = ({ state }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const res = await fetch("http://localhost:4000/update-hospital", {
        method: "POST",
        headers: {
      ...
      if (data.status === 200) {
        ...
        setMessage({
          type: "error",
          text: "Hospital ID is not eligible for update. Please use a different Hospital ID.",
        });
        clearForm();
      } else {
        ...
        <div className="hospital-form-container">
          {isSuccess ? (
            <div className="hospital-success-container">
              ...
              <p>Thank You 😊</p>
              <p>Hospital Update Successful!</p>
            </div>
          ) : (
            <>
